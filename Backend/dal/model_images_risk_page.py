...
class ImagesRiskPage():
    __tablename__ = 'ImagesRiskFactorPage'
    ...
    def add_item(cls, image_url, index):
        ...
        if image_url is not None:
            try:
                conn.execute(f"""
                    insert into ImagesRiskFactorPage
                       ([image]
                       ,[ind])
                    values
                       ('{image_url}'
    ...
    def get_all(cls):
        ...
        result_code = False
        try:
            items = conn.execute("select * from ImagesRiskFactorPage").fetchall()
    ...
    def has_item(cls, image_id):
        ...
        result_code = False
        try:
            query_item = conn.execute(f"select * from ImagesRiskFactorPage where Id = {image_id}").fetchall()[0]
    ...
    def has_item_by_column(cls, column_name, column_value, first_n=None):
        ...
        try:
            if column_value is not None and column_name is not None and len(column_name) > 0:
                items = conn.execute(f"select * from ImagesRiskFactorPage where {column_name} = '{column_value}'").fetchall()
                if items is not None and len(items) > 0:
                    result_code = True
    ...
    def delete_item(cls, index_id):
        ...
        result_code = False
        try:
            conn.execute(f"delete from ImagesRiskFactorPage where ind ={index_id}")
            result_code = True
            conn.commit()
        except Exception as e:
            print(e)
    ...
    def update_item(cls, img_base64, index):
        ...
        result_code = False
        try:
            conn.execute(f"update ImagesRiskFactorPage set image = '{img_base64}' where ind ={index}")
            result_code = True
            conn.commit()
        except Exception as e:
            print(e)
    ...
    def delete_all(cls):
        ...
        result_code = False
        try:
            conn.execute(f"delete from ImagesRiskFactorPage")
            result_code = True
            conn.commit()
        except Exception as e:
            print(e)
